...
function SelectPhase({ items, selectedItems, setSelectedItems, onSubmit }) {
  ...
  return (
    <div className="w-[85%] bg-card-cream/90 border-4 border-border-beige rounded-[35px] px-7 py-5 shadow-xl text-center">
      ...
      <div className="mb-5 inline-block bg-dessert-sec-yellow border-2 border-border-beige rounded-2xl px-5 py-2">
          ...
          return (
            <button
              ...
              className={`relative bg-bg-cream border-2 rounded-3xl shadow-md transition hover:scale-105 flex flex-col items-center justify-center
                ...
                ${isSelected ? 'bg-dessert-selected-blue border-dessert-button-blue' : 'border-border-beige'} `} >
              {isSelected && (
                <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-dessert-pri-blue border-2 border-border-beige flex items-center justify-center">
                  <CheckIcon fontSize="small" />
                </div>
              )}

              <div className={`rounded-full bg-dessert-sec-yellow border-2 border-border-beige flex items-center justify-center ${isHard ? 'w-10 h-10' : isMedium ? 'w-12 h-12' : 'w-14 h-14'}`}>
                <img
                  src={item.image}
                  alt={item.name}
      ...
      <button
        ...
        className={`menu-button ${selectedItems.length === 0 ? 'opacity-50' : ''}`}>
        Submit Order
      </button>
    </div>
  );
