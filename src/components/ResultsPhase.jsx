...
function ResultsPhase({ correctOrder, selectedItems, correctCount, accuracy, currentTheme, onTryAgain, onBack }) {
  return (
    <div className="w-[70%] bg-card-cream/95 border-4 border-border-beige rounded-[35px] px-10 py-8 shadow-xl text-center">
      ...
      <div className="space-x-5">
        <ThemedButton onClick={onTryAgain} currentTheme={currentTheme}>Next Customer</ThemedButton>
        <ThemedButton onClick={onBack} currentTheme={currentTheme}>Go Home</ThemedButton>
      </div>
    </div>
  );
}
